...
LogDAO.prototype.getSensors = function(callback){
	this.adb.collection("v1_config").find({type:'sensor'}).toArray(callback);
}

LogDAO.prototype.getActuators = function(callback){
	this.adb.collection("v1_config").find({type:'actuator'}).toArray(callback);
}
